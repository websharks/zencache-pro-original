...
namespace zencache // Root namespace.
{
	...
	class menu_pages // Plugin options.
	{
		...
		public function options()
		{
			...
			echo '      <p>'.__('<strong>Tip:</strong> ZenCache can be disabled temporarily. If you\'re a theme/plugin developer, you can set a flag within your PHP code to disable the cache engine at runtime. Perhaps on a specific page, or in a specific scenario. In your PHP script, set: <code>$_SERVER[\'QUICK_CACHE_ALLOWED\'] = FALSE;</code> or <code>define(\'QUICK_CACHE_ALLOWED\', FALSE)</code>. ZenCache is also compatible with: <code>define(\'DONOTCACHEPAGE\', TRUE)</code>. It does\'t matter where or when you define one of these, because ZenCache is the last thing to run before script execution ends.', $this->plugin->text_domain).'</p>'."\n";
			echo '      <hr />'."\n";
			echo '      <h3>'.__('Writing "Advanced Cache" Plugins Specifically for ZenCache', $this->plugin->text_domain).'</h3>'."\n";
			echo '      <p>'.__('Theme/plugin developers can take advantage of the ZenCache plugin architecture by creating PHP files inside this special directory: <code>/wp-content/ac-plugins/</code>. There is an <a href="https://github.com/WebSharks/Quick-Cache/blob/000000-dev/zencache/includes/ac-plugin.example.php" target="_blank">example plugin file @ GitHub</a> (please review it carefully and ask questions). If you develop a plugin for ZenCache, please share it with the community by publishing it in the plugins respository at WordPress.org.', $this->plugin->text_domain).'</p>'."\n";
