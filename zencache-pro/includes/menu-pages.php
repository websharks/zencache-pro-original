	...
	class menu_pages // Plugin options.
	{
		...
		public function options()
		{
			...
			if(!$this->plugin->options['enable']) // Not enabled yet?
			{
				...
				echo '   <i class="fa fa-warning"></i> '.__('ZenCache is currently disabled; please review options below.', $this->plugin->text_domain)."\n";
				echo '</div>'."\n";
			}
			...
			echo '      <p>'.__('<strong>To verify that ZenCache is working</strong>, navigate your site like a normal visitor would. Right-click on any page (choose View Source), then scroll to the very bottom of the document. At the bottom, you\'ll find comments that show ZenCache stats and information. You should also notice that page-to-page navigation is <i class="fa fa-flash"></i> <strong>lightning fast</strong> now that ZenCache is running; and it gets faster over time!', $this->plugin->text_domain).'</p>'."\n";
			...
			echo '      <p>'.__('<strong>Tip:</strong> By default, if you delete ZenCache using the plugins menu in WordPress, nothing is lost. However, if you want to completely uninstall ZenCache you should set this to <code>Yes</code> and <strong>THEN</strong> deactivate &amp; delete ZenCache from the plugins menu in WordPress. This way ZenCache will erase your options for the plugin, erase directories/files created by the plugin, remove the <code>advanced-cache.php</code> file, terminate CRON jobs, etc. It erases itself from existence completely.', $this->plugin->text_domain).'</p>'."\n";
			...
			echo '            <option value="1"'.selected($this->plugin->options['uninstall_on_deletion'], '1', FALSE).'>'.__('Yes, uninstall (completely erase) ZenCache on plugin deletion.', $this->plugin->text_domain).'</option>'."\n";
			echo '         </select></p>'."\n";
			echo '   </div>'."\n";
			...
			echo '      <p>'.__('If s2Clean is installed, ZenCache can be configured to clear the Markdown cache too (if you\'ve enabled Markdown processing with s2Clean). The s2Clean Markdown cache is only cleared when you manually clear the cache (with ZenCache); and only if you enable this option here. Note: s2Clean\'s Markdown cache is extremely dynamic. Just like the rest of your site, s2Clean caches do NOT need to be cleared away at all, as this happens automatically when your content changes. However, some developers find this feature useful while developing their site; just to force a refresh.', $this->plugin->text_domain).'</p>'."\n";
			...
			echo '      <p>'.__('If you have other custom routines you\'d like to process when the cache is cleared manually, please type your custom PHP code here. The PHP code that you provide is only evaluated when you manually clear the cache (with ZenCache); and only if the field below contains PHP code. Note: if your PHP code outputs a message (e.g. if you have <code>echo \'&lt;p&gt;My message&lt;/p&gt;\';</code>); your message will be displayed along with any other notes from ZenCache itself. This could be useful to developers that need to clear server caches too (such as <a href="http://www.php.net/manual/en/function.apc-clear-cache.php" target="_blank">APC</a> or <a href="http://www.php.net/manual/en/memcache.flush.php" target="_blank">memcache</a>).', $this->plugin->text_domain).'</p>'."\n";
			...
			echo '            <option value="0"'.selected($this->plugin->options['change_notifications_enable'], '0', FALSE).'>'.__('No, I don\'t want to know (don\'t really care) what ZenCache is doing behind-the-scene.', $this->plugin->text_domain).'</option>'."\n";
			echo '         </select></p>'."\n";
			echo '      <hr />'."\n";
			echo '      <h3>'.__('Auto-Clear Designated "Home Page" Too?', $this->plugin->text_domain).'</h3>'."\n";
			echo '      <p>'.__('On many sites, the Home Page (aka: the Front Page) offers an archive view of all Posts (or even Pages). Therefore, if a single Post/Page is changed in some way; and ZenCache clears/resets the cache for a single Post/Page, would you like ZenCache to also clear any existing cache files for the "Home Page"?', $this->plugin->text_domain).'</p>'."\n";
			...
			echo '      <p>'.__('This is where ZenCache will store the cached version of your site. If you\'re not sure how to deal with directory permissions, don\'t worry too much about this. If there is a problem, ZenCache will let you know about it. By default, this directory is created by ZenCache and the permissions are setup automatically. In most cases there is nothing more you need to do.', $this->plugin->text_domain).'</p>'."\n";
			...
			echo '      <p>'.__('Keep in mind that your Expiration Time is only one part of the big picture. ZenCache will also clear the cache automatically as changes are made to the site (i.e. you edit a post, someone comments on a post, you change your theme, you add a new navigation menu item, etc., etc.). Thus, your Expiration Time is really just a fallback; e.g. the maximum amount of time that a cache file could ever possibly live.', $this->plugin->text_domain).'</p>'."\n";
			echo '      <p>'.__('All of that being said, you could set this to just <code>60 seconds</code> and you would still see huge differences in speed and performance. If you\'re just starting out with ZenCache (perhaps a bit nervous about old cache files being served to your visitors); you could set this to something like <code>30 minutes</code>, and experiment with it while you build confidence in ZenCache. It\'s not necessary to do so, but many site owners have reported this makes them feel like they\'re more-in-control when the cache has a short expiration time. All-in-all, it\'s a matter of preference <i class="fa fa-smile-o"></i>.', $this->plugin->text_domain).'</p>'."\n";
			...
			echo '      <p class="info">'.__('<strong>Advanced Tip:</strong> if you have this set to <code>No</code>, but you DO want to allow a few special URLs to be cached by the browser; you can add this parameter to your URL <code>?zcABC=1</code>. This tells ZenCache that it\'s OK for the browser to cache that particular URL. In other words, the <code>zcABC=1</code> parameter tells ZenCache NOT to send no-cache headers to the browser.', $this->plugin->text_domain).'</p>'."\n";
			echo '   </div>'."\n";
			...
			echo '      <p class="info">'.__('<strong>Note:</strong> This setting includes some users who AREN\'T actually logged into the system, but who HAVE authored comments recently. ZenCache includes comment authors as part of it\'s logged-in user check. This way comment authors will be able to see updates to the comment thread immediately; and, so that any dynamically-generated messages displayed by your theme will work as intended. In short, ZenCache thinks of a comment author as a logged-in user, even though technically they are not. ~ Users who gain access to password-protected Posts/Pages are also included.', $this->plugin->text_domain).'</p>'."\n";
			echo '   </div>'."\n";
			...
			echo '      <p class="info">'.__('<strong>Advanced Tip:</strong> If you are NOT caching GET requests (recommended), but you DO want to allow some special URLs that include query string parameters to be cached; you can add this special parameter to any URL <code>?zcAC=1</code>. This tells ZenCache that it\'s OK to cache that particular URL, even though it contains query string arguments. If you ARE caching GET requests and you want to force ZenCache to NOT cache a specific request, you can add this special parameter to any URL <code>?zcAC=0</code>.', $this->plugin->text_domain).'</p>'."\n";
			echo '   </div>'."\n";
			...
			echo '      <p>'.__('When this is set to <code>No</code>, ZenCache will ignore all 404 requests and no cache file will be served. While this is fine for most site owners, caching the 404 page on a high-traffic site may further reduce server load. When this is set to <code>Yes</code>, ZenCache will cache the 404 page (see <a href="https://codex.wordpress.org/Creating_an_Error_404_Page" target="_blank">Creating an Error 404 Page</a>) and then serve that single cache file to all future 404 requests.', $this->plugin->text_domain).'</p>'."\n";
			...
			if(is_multisite()) echo '<p class="info" style="display:block; margin-top:-15px;">'.__('In a Multisite Network, each child blog will be auto-cached too. ZenCache will dynamically change the leading <code>'.esc_html(home_url('/')).'</code> as necessary; for each child blog in the network. ZenCache supports both sub-directory &amp; sub-domain networks; including domain mapping plugins.', $this->plugin->text_domain).'</p>'."\n";
			...
			echo '      <p>'.__('This feature allows you to serve some and/or ALL static files on your site from a CDN of your choosing. This is made possible through content/URL filters exposed by WordPress and implemented by ZenCache. All it requires is that you setup a CDN host name sourced by your WordPress installation domain. You enter that CDN host name below and ZenCache will do the rest! Super easy, and it does\'nt require any DNS changes either :-) Please <a href="https://github.com/websharks/zencache/wiki/Static-CDN-Filters-(General-Instructions)" target="_blank">click here</a> for a general set of instructions.', $this->plugin->text_domain).'</p>'."\n";
			...
			echo '         <p>'.__('Each filtered URL (which then leads to your CDN) will include this query string variable as an easy way to invalidate the CDN cache at any time. Invalidating the CDN cache is simply a matter of changing the global invalidation counter (i.e. the value assigned to this query string variable). ZenCache manages invalidations automatically; i.e. ZenCache will automatically bump an internal counter each time you upgrade a WordPress component (e.g. a plugin, theme, or WP itself). Or, if you ask ZenCache to invalidate the CDN cache (e.g. a manual clearing of the CDN cache); the internal counter is bumped then too. In short, ZenCache handles cache invalidations for you reliably. This option simply allows you to customize the query string variable name which makes cache invalidations possible. <strong>Please note, the default value is adequate for most sites. You can change this if you like, but it\'s not necessary.</strong>', $this->plugin->text_domain).'</p>'."\n";
			echo '         <p class="info" style="display:block;">'.__('<strong>Note:</strong> If you empty this field, it will effectively disable the ZenCache invalidation system for Static CDN Filters; i.e. the query string variable will NOT be included if you do not supply a variable name.', $this->plugin->text_domain).'</p>'."\n";
			echo '      </div>'."\n";
			echo '   </div>'."\n";
			...
			echo '      <p>'.__('<em>Note: Understanding the ZenCache <a href="http://www.websharks-inc.com/r/zencache-branched-cache-structure-wiki/" target="_blank">Branched Cache Structure</a> is a prerequisite to understanding how Dynamic Version Salts are added to the mix.</em>', $this->plugin->text_domain).'</p>'."\n";
			...
			echo '      <p>'.__('For more documentation, please see <a href="http://www.websharks-inc.com/r/zencache-dynamic-version-salts-wiki/" target="_blank">Dynamic Version Salts</a>.', $this->plugin->text_domain).'</p>'."\n";
			echo '      <hr />'."\n";
			echo '      <h3>'.__('Create a Dynamic Version Salt For ZenCache? &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <span style="font-size:90%; opacity:0.5;">150% OPTIONAL</span>', $this->plugin->text_domain).'</h3>'."\n";
			...
			echo '      <p>'.__('<strong>Tip:</strong> ZenCache can be disabled temporarily. If you\'re a theme/plugin developer, you can set a flag within your PHP code to disable the cache engine at runtime. Perhaps on a specific page, or in a specific scenario. In your PHP script, set: <code>$_SERVER[\'ZENCACHE_ALLOWED\'] = FALSE;</code> or <code>define(\'ZENCACHE_ALLOWED\', FALSE)</code>. ZenCache is also compatible with: <code>define(\'DONOTCACHEPAGE\', TRUE)</code>. It does\'t matter where or when you define one of these, because ZenCache is the last thing to run before script execution ends.', $this->plugin->text_domain).'</p>'."\n";
			echo '      <hr />'."\n";
			echo '      <h3>'.__('Writing "Advanced Cache" Plugins Specifically for ZenCache', $this->plugin->text_domain).'</h3>'."\n";
			echo '      <p>'.__('Theme/plugin developers can take advantage of the ZenCache plugin architecture by creating PHP files inside this special directory: <code>/wp-content/ac-plugins/</code>. There is an <a href="https://github.com/websharks/zencache/blob/000000-dev/zencache/includes/ac-plugin.example.php" target="_blank">example plugin file @ GitHub</a> (please review it carefully and ask questions). If you develop a plugin for ZenCache, please share it with the community by publishing it in the plugins respository at WordPress.org.', $this->plugin->text_domain).'</p>'."\n";
			echo '      <p class="info">'.__('<strong>Why does ZenCache have it\'s own plugin architecture?</strong> WordPress loads the <code>advanced-cache.php</code> drop-in file (for caching purposes) very early-on; before any other plugins or a theme. For this reason, ZenCache implements it\'s own watered-down version of functions like <code>add_action()</code>, <code>do_action()</code>, <code>add_filter()</code>, <code>apply_filters()</code>.', $this->plugin->text_domain).'</p>'."\n";
			echo '   </div>'."\n";
			...
			echo '      <h3>'.__('Import Options from Another ZenCache Installation?', $this->plugin->text_domain).'</h3>'."\n";
			...
			echo '      <hr />'."\n";
			echo '      <h3>'.__('Export Existing Options from this ZenCache Installation?', $this->plugin->text_domain).'</h3>'."\n";
			echo '      <button type="button" class="plugin-menu-page-export-options" style="float:right; margin: 0 0 0 25px;"'. // Exports existing ZenCache options from this installation.
			...
			echo '      <p>'.__('Download your existing options and import them all into another ZenCache installation; saves time on future installs.', $this->plugin->text_domain).'</p>'."\n";
			echo '   </div>'."\n";
		...
		public function update_sync()
		{
			...
			echo '      <p>'.__('When a new version of ZenCache Pro becomes available, WebSharks™ can display a notification in your WordPress Dashboard prompting you to return to this page and perform an upgrade. Would you like this functionality enabled or disabled?', $this->plugin->text_domain).'</p>'."\n";
			echo '      <hr />'."\n";
			...
			echo '            <option value="0"'.selected($this->plugin->options['update_sync_version_check'], '0', FALSE).'>'.__('No, do not display any ZenCache update notifications in my WordPress Dashboard.', $this->plugin->text_domain).'</option>'."\n";
			echo '         </select></p>'."\n";
			echo '   </div>'."\n";
